...
// CounterGaugeCell which has a null implementation.
...
class Counter {
 public:
  ...
  CounterGaugeCell* GetCell(const Labels&... labels) {
 ...
 private:
  ...
  CounterGaugeCell default_counter_cell_;
...
class CounterGauge {
 ...
 private:
  ...
  std::map<LabelArray, CounterGaugeCell> cells_ ABSL_GUARDED_BY(mu_);
