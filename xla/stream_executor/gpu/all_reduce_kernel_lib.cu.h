...
template <typename T>
__device__ __forceinline__ RestrictedPtr<T> GetPeerBufferPtr(
...
__device__ __forceinline__ void OneShotAllReduceKernelImpl(
  ...
  if (threadIdx.x < args.num_ranks) {
    remote_input_buffers[threadIdx.x] =
        GetPeerBufferPtr<T>(args.symmetric_input_ptrs, threadIdx.x, 0,
                            args.num_ranks, *args.metadata);
    signal_flags_buffers[threadIdx.x] =
        GetPeerBufferPtr<uint32_t>(args.symmetric_signal_ptrs, threadIdx.x, 1,
                                   args.num_ranks, *args.metadata);
  }
