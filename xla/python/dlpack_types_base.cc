...
absl::StatusOr<nanobind::dlpack::dtype> PrimitiveTypeToNbDLDataType(
  ...
  absl::StatusOr<DLDataType> dl_type_status = PrimitiveTypeToDLDataType(type);
  if (!dl_type_status.ok()) {
    return dl_type_status.status();
  }
  DLDataType dl_type = *dl_type_status;
