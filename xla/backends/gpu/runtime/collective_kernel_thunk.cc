...
absl::Status CollectiveKernelThunk::ExchangeStateMetadata(
  ...
  std::vector<uint64_t> param_to_peers_ptrs(rendezvous_values->size() * 2);
  for (int i = 0; i < rendezvous_values->size(); ++i) {
    param_to_peers_ptrs.emplace_back(reinterpret_cast<uint64_t>(
        rendezvous_values->at(i).buffer_ptr.opaque()));
  }
  for (int i = 0; i < rendezvous_values->size(); ++i) {
    param_to_peers_ptrs.emplace_back(reinterpret_cast<uint64_t>(
        rendezvous_values->at(i).signal_ptr.opaque()));
  }

  size_t peer_to_peers_ptrs_size =
      param_to_peers_ptrs.size() * sizeof(uint64_t);
  se::DeviceMemoryBase metadata_ptr = params.executor->Allocate(
      sizeof(CollectiveKernelMetadata) + peer_to_peers_ptrs_size, 0);
  se::DeviceMemoryBase param_to_peers_ptrs_buffer = metadata_ptr.GetByteSlice(
      sizeof(CollectiveKernelMetadata), peer_to_peers_ptrs_size);
