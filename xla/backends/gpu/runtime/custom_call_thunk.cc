...
absl::StatusOr<ThunkProto> CustomCallThunk::ToProto() const {
  ...
  *proto.mutable_thunk_info() = thunk_info().ToProto();
  ...
  for (const auto& operand : operands_) {
    auto* operand_proto = proto.mutable_custom_call_thunk()->add_operands();
    if (operand.has_value()) {
      TF_ASSIGN_OR_RETURN(*operand_proto, operand->ToProto());
    } else {
      TF_ASSIGN_OR_RETURN(*operand_proto, ShapedSlice{}.ToProto());
    }
  }

  for (const auto& result : results_) {
    auto* result_proto = proto.mutable_custom_call_thunk()->add_results();
    if (result.has_value()) {
      TF_ASSIGN_OR_RETURN(*result_proto, result->ToProto());
    } else {
      TF_ASSIGN_OR_RETURN(*result_proto, ShapedSlice{}.ToProto());
    }
  }
...
absl::StatusOr<std::unique_ptr<CustomCallThunk>> CustomCallThunk::FromProto(
  ...
  std::vector<std::optional<ShapedSlice>> operands, results;
  for (const auto& operand_proto : proto.operands()) {
    TF_ASSIGN_OR_RETURN(
        ShapedSlice operand,
        ShapedSlice::FromProto(operand_proto, buffer_allocations));
    operands.push_back(std::move(operand));
  }
  for (const auto& result_proto : proto.results()) {
    TF_ASSIGN_OR_RETURN(
        ShapedSlice result,
        ShapedSlice::FromProto(result_proto, buffer_allocations));
